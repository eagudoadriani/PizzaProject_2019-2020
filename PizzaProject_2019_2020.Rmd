...
bootFUN <- function(model, newdata) {
  ...
  predict(update, newdata, type = "response") # predicts response variable with model and updated data 
}
...
AlgaeWeight_Warm2020_boot <- cbind(AlgaeWeight_Warm2020_fishRM, as.data.frame(t(apply(boot, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
...
ggsave("AlgaeWeight_bootCI.png", width = 22, height = 10, units = "cm", dpi = 300)
...
AlgaeWeight_Warm2020_boot<-AlgaeWeight_Warm2020_boot %>% filter(Treatment %in% c("Open pizzas", "Procedural Control")) 
