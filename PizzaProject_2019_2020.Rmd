...
```{r}
#Loading data
AlgaeWeight_Cold2019<-read.csv(file="AlgaeWeight_Cold2019.csv") # I changed this to 'AlgaeWeight_Cold2020.csv' from 'AlgaeWeight_Cold2019.csv' since that file did not exist
...
bootFUN <- function(model, newdata) {
  ...
  data2 <- data[sample(1:nr, size = nr, replace = TRUE), ] # random sample of numbers from 1 - nr (# of rows) to select data from
  update <- update(model, data = data2) # rerun the model using the 'new' dataset from the random smapling above
  predict(update, newdata = newdata, type = "response", allow.new.levels=TRUE) # predicts response variable with model and updated data 
}
...
```
>>>>>>> 3537efe963c824661a04076f25751f60d25af297
