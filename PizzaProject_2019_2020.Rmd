...
AlgaeWeight_Cold2019<-AlgaeWeight_Cold2019 %>%
  filter(Treatment!="Fish access")%>%
  filter(Sample.weight.postburn..g.!=0)
...
warm_gamma_table
```
## Bootstraping to get bootstraping means and CI. 
...
set.seed(12345)
library(boot)
myBootstrap<-boot(AlgaeWeight_Cold2019, foo, R=1000, strata=AlgaeWeight_Cold2019$Treatment)
```
...
### Cold and warm CI plot

```{r combined CI plot}

plot_grid(cold_boot_plot, warm_boot_plot, labels = c("C", "D"))
