...
cold_2019_update <- cold_2019 %>% 
  ...
  mutate(Algae = Algae + 0.01) # add offset for gamma GLM


## Cold season GLM for algal cover
cold_cover_glm <- glm(Algae ~ Treatment * Week + N, family = Gamma(link = "log"), maxit = 100, data = cold_2019_update)
...
warm_2020_update <- warm_2020 %>% 
  mutate(Algae = as.numeric(Algae + 0.02)) # add offset for gamma GLM
  

## Warm season GLM for algal cover
warm_cover_glm <- glm(Algae ~ Treatment + Week + N, family = Gamma(link = "log"), maxit = 100, data = warm_2020_update)
...
boot_cover2 <- replicate(bootnum, bootFUN(model = warm_cover_glm, newdata = newdata)) # run the bootFUN the number (bootnum) of times specified and save as matrix
