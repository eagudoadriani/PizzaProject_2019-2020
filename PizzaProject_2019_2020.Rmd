...
AlgaeWeight_Cold2019$N <- factor(AlgaeWeight_Cold2019$N, levels = c("N+", "No N"))
...
#Reordering and renaming Factor: Nutrient
AlgaeWeight_Warm2020$N <- factor(AlgaeWeight_Warm2020$N, levels = c("N+", "No N"))
...
levels(AlgaeWeight_Warm2020$Treatment)<-(c("All present"="Open Cages", "Urchins Only"="Fish exclusion", "None Present"="Full exclusion"))
...
Comparison between the effect of herbivore exclussion and nutrient addition across cold season and warm season. 
```{r}
...
warm_season_model_gamma <- glm((AFDW..g...preburn..postburn.) ~ Treatment * N, family = Gamma(link="log"), data = AlgaeWeight_Warm2020)
...
# need to remove fish exclusion here before running model
...
warm_glm <- glm(AFDW..g...preburn..postburn. ~ Treatment * N, family = Gamma(link = "log"), data = AlgaeWeight_Warm2020)
...
colnames(AlgaeWeight_Warm2020_boot)[13:15] <- c("mean", "lowerci", "upperci") # rename mean/CI columns
...
warm_boot_plot <- ggplot() +
  ...
  xlab("") +
  ggtitle("Warm Season") +
  scale_y_continuous(expand=c(0,0), breaks=seq(0, 15, 1), limits = c(0, 100)) + # makes the y-axis display as $
...
mean_AlgaeWeight_Cold2019_boot<-AlgaeWeight_Cold2019_boot%>%
  group_by(Treatment, N)%>%
  summarise(mean_weight=mean(AFDW..g...preburn..postburn.), mean_BS=mean(mean), lowerc=mean(lowerci), upperci=mean(upperci))
...
write.csv(mean_AlgaeWeight_Cold2019_boot, file="Tables.csv", #sheetName = "Cold_WeightBoot",
          col.names = TRUE, row.names = TRUE, append = FALSE)

write.csv(mean_AlgaeWeight_Warm2020_boot, file="Tables.csv", #sheetName = "Warm_WeightBoot",
          col.names = TRUE, row.names = TRUE, append = TRUE)

write.csv(summary_cold_glm, file="Tables.csv", #sheetName = "cold_glm",  
          col.names = TRUE, row.names = TRUE, append = TRUE)

write.csv(summary_warm_glm 
, file="Tables.csv", #sheetName = "warm_glm",
col.names = TRUE, row.names = TRUE, append = TRUE)

```

## Colleen's Bootstrapping of % algal cover
...
cold_2019_boot <- cbind(cold_2019_update, as.data.frame(t(apply(boot_cover, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
colnames(cold_2019_boot)[21:23] <- c("mean", "lowerci", "upperci") # rename mean/CI columns

```

```{r plot cool season % cover boot, echo=FALSE}
...
``` {r save cool cover plot}

Algae_CoverCoolSeason_boot

png("Algae_CoverCoolSeason_boot.png", width=14, height=8, units="cm", res=300)
print(Algae_CoverCoolSeason_boot)
dev.off()
...
# Calculate the mean, 95% lowerCI, and 95% upperCI from the boot_cover matrix and add it to dataframe (but as a new name)
warm_2020_boot <- cbind(warm_2020_update, as.data.frame(t(apply(boot_cover2, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
colnames(warm_2020_boot)[19:21] <- c("mean", "lowerci", "upperci") # rename mean/CI columns

```


```{r plot warm season % cover boot, echo=FALSE}
...
``` {r save warm cover plot}

Algae_CoverwarmSeason_boot

png("AlgaeCover_WarmSeason_boot.png", width=14, height=8, units="cm", res=300)
print(Algae_CoverwarmSeason_boot)
dev.off()
...
png("ComparationAlgaeCover_boot.png", width=14, height=8, units="cm", res=300)
print(AlgaeCover_BothSeason_Boot)
dev.off()
...
Warm_ControlProcedure <- ggplot() +
  theme_bw() + # sets plot theme (removes gridlines, etc.)
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", size= 0.8))+
  geom_point(data = AlgaeWeight_Warm2020_boot, aes(x = N , y = AFDW..g...preburn..postburn., colour = Treatment), shape = 1, alpha = 0.4, position = position_jitterdodge(jitter.width = NULL, jitter.height = 0,dodge.width = 0.85)) +
...
### Session Information from last compile run:
