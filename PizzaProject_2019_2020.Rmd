...
levels(cold_2019$Treatment)<-(c("All present"="Open Cages", "Fish only"="Fish access", "Urchins Only"="Fish exclusion", "None present"="Full exclusion"))
```
...
levels(warm_2020$Treatment)<-(c("All present"="Open Cages",  "Urchins Only"="Fish exclusion", "None present"="Full exclusion"))
```
### Plotting_WARM
```{r, fig.height=5, fig.width=10}
...
```{r}
Cover_BothSeason<-ggarrange(Algae_CoverColdSeason
,AlgaeCover_HotSeason, nrow=2, common.legend = TRUE, labels = c("A", "B"), hjust = -0.1, vjust = 1.5, legend="bottom")

png("Cover_BothSeason.png", width=16, height=15, units="cm", res=300)
print(Cover_BothSeason)
dev.off()
...
### Cold Season
#```{r cold cover bootstraping}

## Cold season GLM
cold_cover_glm <- glm((Algae + 0.0001) ~ Treatment * N * as.integer(Week), family = Gamma(link = "log"), data = cold_2019)

## Run parametric bootstrapping:
newdata <- data.frame(Treatment = cold_2019$Treatment, N = cold_2019$N, Week = cold_2019$Week) # creates dataframe of predictor variables (here, Treatment and N)
boot <- replicate(bootnum, bootFUN(model = cold_cover_glm, newdata = newdata)) # run the bootFUN the number (bootnum) of times specified and save as matrix

# Calculate the mean, 95% lowerCI, and 95% upperCI from the boot matrix and add it to dataframe (but as a new name)
cold_2019_boot <- cbind(cold_2019, as.data.frame(t(apply(boot, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
colnames(cold_2019_boot)[22:24] <- c("mean", "lowerci", "upperci") # rename mean/CI columns

#```

#```{r cool cover boot plot}

### Plot the observational values and bootstrapped mean/CI
cold_cover_boot_plot <- ggplot() +
  theme_bw() + # sets plot theme (removes gridlines, etc.)
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), legend.position="none")+
  geom_point(data = cold_2019_boot, aes(x = Week , y = Algae, colour = Treatment), shape = 1, alpha = 0.4, position = position_jitterdodge(jitter.width = NULL, jitter.height = 0,dodge.width = 0.5)) +
  geom_linerange(data = cold_2019_boot, aes(x = Week, ymin = lowerci, ymax = upperci, colour = Treatment), position = position_dodge(width = 0.5)) +
  geom_line(data = cold_2019_boot, aes(x = Week, y = mean, colour = Treatment, group = Treatment), position = position_dodge(width = 0.5)) +
  geom_point(data = cold_2019_boot, aes(x = Week, y = mean, colour = Treatment), position = position_dodge(width = 0.5)) + # plots the mean value 
  scale_colour_manual(name = "", values = colour.palette) + # adds the colours for plotting
  ylab("Mean algal cover (%)") + # y-axis label
  xlab("Sampling Week") +
  ggtitle("Cool Season") +
...
### Warm Season -- This is not working yet, but wanted to upload things as they were at the moment
#```{r warm cover bootstraping}

## Warm season GLM
warm_cover_glm <- glm((Algae + 0.001) ~ Treatment * N * Week, family = Gamma(link = "log"), data = warm_2020)

## Run parametric bootstrapping:
newdata <- data.frame(Treatment = warm_2020$Treatment, N = warm_2020$N, Week = warm_2020$Week) # creates dataframe of predictor variables (here, Treatment and N)
boot <- replicate(bootnum, bootFUN(model = warm_cover_glm, newdata = newdata)) # run the bootFUN the number (bootnum) of times specified and save as matrix

# Calculate the mean, 95% lowerCI, and 95% upperCI from the boot matrix and add it to dataframe (but as a new name)
warm_2020_boot <- cbind(warm_2020, as.data.frame(t(apply(boot, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
colnames(warm_2020_boot)[22:24] <- c("mean", "lowerci", "upperci") # rename mean/CI columns

#```

#```{r warm cover boot plot}

### Plot the observational values and bootstrapped mean/CI
warm_cover_boot_plot <- ggplot() +
  theme_bw() + # sets plot theme (removes gridlines, etc.)
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), legend.position="none")+
  geom_point(data = warm_2020_boot, aes(x = Week , y = Algae, colour = Treatment), shape = 1, alpha = 0.4, position = position_jitterdodge(jitter.width = NULL, jitter.height = 0,dodge.width = 0.5)) +
  geom_linerange(data = warm_2020_boot, aes(x = Week, ymin = lowerci, ymax = upperci, colour = Treatment), position = position_dodge(width = 0.5)) +
  geom_line(data = warm_2020_boot, aes(x = Week, y = mean, colour = Treatment, group = Treatment), position = position_dodge(width = 0.5)) +
  geom_point(data = warm_2020_boot, aes(x = Week, y = mean, colour = Treatment), position = position_dodge(width = 0.5)) + # plots the mean value 
  scale_colour_manual(name = "", values = colour.palette) + # adds the colours for plotting
  ylab("Mean algal cover (%)") + # y-axis label
  xlab("Sampling Week") +
  ggtitle("Warm Season") +
...
levels(AlgaeWeight_Cold2019$Treatment)<-(c("All present"="Open Cages", "Fish only"="Fish access", "Urchins Only"="Fish exclusion", "None present"="Full exclusion"))


levels(AlgaeWeight_Warm2020$Treatment)<-(c("All present"="Open Cages", "Urchins Only"="Procedural Control", "None present"="Full exclusion"))
...
#I included this to remove the urching access treatment. 

AlgaeWeight_Warm2020_boot_NU<-AlgaeWeight_Warm2020_boot %>%
  filter(Treatment %in% c("Open Cages", "Full exclusion"))
...
warm_boot_plot <- ggplot() +
  ...
  geom_point(data = AlgaeWeight_Warm2020_boot_NU, aes(x = N , y = Sample.weight.postburn..g., colour = Treatment), shape = 1, alpha = 0.4, position = position_jitterdodge(jitter.width = NULL, jitter.height = 0,dodge.width = 0.85)) +
  geom_linerange(data = AlgaeWeight_Warm2020_boot_NU, aes(x = N, ymin = lowerci, ymax = upperci, colour = Treatment), alpha = 0.1, size = 8, position = position_dodge(width = 0.85)) +
  geom_point(data = AlgaeWeight_Warm2020_boot_NU, aes(x = N, y = mean, colour = Treatment), shape = "-", size = 20, position = position_dodge(width = 0.85)) + # plots the mean value 
  scale_colour_manual(name = "", values = colour.palette) + # adds the colours for plotting
...
### Cold and warm Bootstrap CI plot_CD
...
AlgaeWeight_bootCI<-ggarrange(cold_boot_plot, warm_boot_plot, nrow=1, labels = c("C", "D "),legend="bottom")
...
```{r}
Plot_bootstrapCI<-plot_grid(cold_boot_plot, warm_boot_plot, labels = c("A", "B"))
Plot_bootstrapCI

AlgaeWeight_bootCI<-ggarrange(cold_boot_plot, warm_boot_plot, nrow=1, labels = c("A", "B"),legend="bottom")

png("AlgaeWeight_bootCI_AB.png", width=17, height=10, units="cm", res=300)
print(AlgaeWeight_bootCI)
dev.off()
```
...
summary_cold_glm<-data.frame(coef(summary(cold_glm)))
summary_warm_glm<-data.frame(coef(summary(warm_glm))) 
...
write.xlsx(summary_cold_glm, file="Tables.xlsx", sheetName = "cold_glm",  col.names = TRUE, row.names = TRUE, append = TRUE)

write.xlsx(summary_warm_glm 
, file="Tables.xlsx", sheetName = "warm_glm",  col.names = TRUE, row.names = TRUE, append = TRUE)
...
#Comparing control open cage with procedural control 

AlgaeWeight_Warm2020_boot<-AlgaeWeight_Warm2020_boot %>% filter(Treatment %in% c("Open Cages", "Procedural Control")) 

Warm_ControlProcedure <- ggplot() +
  theme_bw() + # sets plot theme (removes gridlines, etc.)
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", size= 0.8))+
  ...
  xlab("") +
  ggtitle("Warm Season") +
  scale_y_continuous(expand=c(0,0), breaks=seq(0, 7, 1), limits = c(0, 8)) + # makes the y-axis display as $
  theme(axis.text.x = element_text(size = 9.5), axis.title.y = element_text(size = 9.5, hjust = 0.6, vjust = 2.5), legend.position = "bottom", legend.text = element_text(size = 10), legend.key.size = unit(0.5, "cm"), legend.key.height = unit(0.5, "cm")) + #Legend theme
  guides(colour = guide_legend(override.aes = list(shape = 15, size=3)))

png("WarmProceduralControl.png", width=8, height=10, units="cm", res=300)
print(Warm_ControlProcedure)
dev.off()

Warm_ControlProcedure
```
